...
class Metric:
    def __init__(self,labels_list, predictions, anomaly_map_list, gt_list, config) -> None:
        ...
        # self.config = config
        # self.threshold = 0.5
        self.threshold = self.optimal_threshold()
    ...
    def optimal_threshold(self):   # 找到最大化J统计量的最优分类阈值
    ...
    def get_predictions(self):  # 根据最优阈值将预测结果二值化（正常/异常）
        # 将预测值二值化(0=正常,1=异常)
        predictions = torch.tensor(self.predictions) 
        binary_preds = (predictions > self.threshold).int().tolist()
        return binary_preds
